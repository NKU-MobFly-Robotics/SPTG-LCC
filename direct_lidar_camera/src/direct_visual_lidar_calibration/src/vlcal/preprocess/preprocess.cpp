...
bool Preprocess::Preprocess_init(int argc, char** argv, int status_flag, Eigen::Isometry3d init_L_C)
{
  ...
  options_description description("preprocess");
   std::vector<double> matrix_T_L0C0,matrix_T_CL_init;
    ...
    double min_dis;
    nh_.param< double >("preprocess/min_dis",min_dis,0.5);
 ...
 std::cout<<"ddddddddd"<<min_dis<<std::endl;
  ...
  view_culling_params.enable_depth_buffer_culling = true;
...
cv::compare(result, 0.1, mask2, cv::CMP_LT); 
   ...
   cv::Mat dilatedImage2;
    ...
    // Median blur
      cv::Mat blurredImage;
      cv::GaussianBlur(result2, blurredImage, cv::Size(5, 5), 0);
...
void gammaCorrection(  cv::Mat& src, cv::Mat& dst, float gamma) {
    // Convert image to float type for gamma correction
    cv::Mat floatImg;
    src.convertTo(floatImg, CV_32F, 1.0 / 255.0);

    // Apply gamma correction
    cv::pow(floatImg, gamma, floatImg);
...
void histogramMatching(  cv::Mat& source,  cv::Mat& tmplImage, cv::Mat& result) {
    // Ensure that the images are single channel
    CV_Assert(source.type() == CV_8UC1 && tmplImage.type() == CV_8UC1);

    // Compute the histogram of the source and template images
    int histSize = 256;
    float range[] = { 0, 256 };
    const float* histRange = { range };
    cv::Mat sourceHist, tmplHist;
    cv::calcHist(&source, 1, 0, cv::Mat(), sourceHist, 1, &histSize, &histRange);
    cv::calcHist(&tmplImage, 1, 0, cv::Mat(), tmplHist, 1, &histSize, &histRange);

    // Normalize the histograms
    sourceHist /= source.total();
    tmplHist /= tmplImage.total();

    // Compute the cumulative distribution function (CDF) for both histograms
    cv::Mat sourceCDF(sourceHist.size(), sourceHist.type());
    cv::Mat tmplCDF(tmplHist.size(), tmplHist.type());
    sourceCDF.at<float>(0) = sourceHist.at<float>(0);
    tmplCDF.at<float>(0) = tmplHist.at<float>(0);
    for (int i = 1; i < histSize; i++) {
        sourceCDF.at<float>(i) = sourceCDF.at<float>(i - 1) + sourceHist.at<float>(i);
        tmplCDF.at<float>(i) = tmplCDF.at<float>(i - 1) + tmplHist.at<float>(i);
    }

    // Create a lookup table to map source pixel values to template pixel values
    cv::Mat lut(1, 256, CV_8U);
    for (int i = 0; i < 256; i++) {
        float F1 = sourceCDF.at<float>(i);
        int j = 0;
        while (j < 256 && tmplCDF.at<float>(j) < F1) {
            j++;
        }
        lut.at<uchar>(i) = j;
    }

    // Apply the lookup table to the source image to get the result image
...
bool Preprocess::run(int argc, char** argv, int status_flag, Eigen::Isometry3d init_L_C)
 ...
 if(initial_alignment==true&&status_flag==0)
  { T_lidar_camera.linear()=T_CL_init.inverse().linear();}
 else if(initial_alignment==false&&status_flag==0)
 {
     T_lidar_camera.linear() = (Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(-M_PI_2, Eigen::Vector3d::UnitZ())).toRotationMatrix();
 }
 else if(status_flag>0)
 {
  std::cout<<"init_L_C.linear()"<<init_L_C.linear()<<std::endl;
    std::cout<<"T_CL_init.linear()"<<T_CL_init.linear()<<std::endl;
    T_lidar_camera.linear()=init_L_C.inverse().linear();//init_L_C.linear();
 }
 ...
 {
   T_lidar_camera.linear()=init_L_C.inverse().linear();
 }
    // T_lidar_camera.linear() = (Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(-M_PI_2, Eigen::Vector3d::UnitZ())).toRotationMatrix();
    const double fx = lidar_image_size.x() / (2.0 * std::tan(lidar_fov / 2.0));
    lidar_camera_model = "plumb_bob";
  ...
  #pragma omp parallel for
    ...
    //计时器
     auto start = std::chrono::system_clock::now();
      ...
      auto [intensities_eq,depth_image_eq,indices_eq] = vlcal::generate_lidar_image(lidar_proj_equirectangular, lidar_image_size_equirectangular, T_lidar_camera.inverse(), lidar_points[i]);
            
auto end2 = std::chrono::system_clock::now();
         auto duration2 = std::chrono::duration_cast<std::chrono::milliseconds>(end2 - start);

            std::cout << "elapsed time3: " << duration2.count() << " [ms]" << std::endl; 
...
//  index_image.at<std::int32_t>(pt_2d.y(), pt_2d.x()) = i;
 ...
 cv::Mat copy_cropped_depth_save=cropped_depth.clone() ;
    ...
    cv::Mat depth_light_eq,depth_light;
 
//    if(status_flag>7)
//    {  
    
//    cv::Mat source=cv::imread(dst_path + "/" + bag_name + "_rgb_pred.png", cv::IMREAD_GRAYSCALE);
//    gammaCorrection(  depth_fill_in_fast, depth_light , 0.7) ;
//       gammaCorrection(  depth_fill_in_fast_eq, depth_light_eq , 0.7) ;

//     histogramMatching(depth_fill_in_fast, source , depth_light);
//       histogramMatching( depth_fill_in_fast_eq ,source, depth_light_eq);
//       depth_light=depth_fill_in_fast;

// depth_light_eq=depth_fill_in_fast_eq;
//    } 
//    else
//    {depth_light=depth_fill_in_fast;
...
//    }
    ...
    auto end = std::chrono::system_clock::now();
        ...
        std::cout << "elapsed time2: " << duration.count() << " [ms]" << std::endl; 
    ...
    //     // 计算已经过的时间
    //     auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(end - start2).count();
...
int point_num=0;
  ...
  while (true) {
   ...
   vlcal::RawPoints raw_points_select;
  ...
  //   }
    ...
    if (p.norm() < 100.0&&p.norm()>2.5) {
// if (p.norm() < 80.0) {
raw_points_select.times.push_back(raw_points->times[i]);
raw_points_select.points.push_back(raw_points->points[i]);
raw_points_select.intensities.push_back(raw_points->intensities[i]);
    }
  ...
  //   }
    ...
    // 
    auto points = std::make_shared<FrameCPU>(raw_points_select.points);
    points->add_times(raw_points_select.times);
    points->add_intensities(raw_points_select.intensities);
    points = filter(points, [](const Eigen::Vector4d& p) { return p.array().isFinite().all(); });
raw_points_select.points.clear();
raw_points_select.intensities.clear();
raw_points_select.times.clear();
