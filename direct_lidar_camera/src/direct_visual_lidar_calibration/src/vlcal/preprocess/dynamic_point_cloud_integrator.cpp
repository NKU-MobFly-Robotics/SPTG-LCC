...
double rotationDistance(const gtsam::Rot3& r1, const gtsam::Rot3& r2) {
    gtsam::Quaternion q1 = r1.toQuaternion();
    gtsam::Quaternion q2 = r2.toQuaternion();
    double dot_product = q1.dot(q2);
    return std::acos(std::min(1.0, std::max(-1.0, dot_product)));
}
...
#pragma omp parallel for num_threads(params.num_threads)
  ...
  if (!target_ivox->has_points()) {
    ...
    last_pose = gtsam::Pose3();  // 初始化前一帧的位姿
    return;
  }
  ...
  // 检测里程计是否有变化
  gtsam::Pose3 current_pose = values.at<gtsam::Pose3>(1);
  double translation_change = (current_pose.translation() - last_pose.translation()).norm();
  double rotation_change = rotationDistance(current_pose.rotation(), last_pose.rotation());

  double translation_threshold = 0.005; // 平移变化的阈值
  double rotation_threshold = 0.002; // 旋转变化的阈值

  bool has_moved = (translation_change > translation_threshold) || (rotation_change > rotation_threshold);
...
auto deskewed = std::make_shared<FrameCPU>(factor->deskewed_source_points(values));
 ...
 {flag_static=0;}
  ...
  target_ivox->insert(*deskewed); 
    std::cout << "Sensor has moved." << std::endl;
