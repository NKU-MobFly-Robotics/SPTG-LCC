...
: draw_sphere(draw_sphere),
  ...
  image_display_scale = std::min(1920.0 / dataset[0]->image2.cols, 1080.0 / dataset[0]->image2.rows);
...
void VisualLiDARVisualizer::ui_callback() {
  ...
  if (prev_selected_bag_id != selected_bag_id) {
    ...
    color_updater.reset(new PointsColorUpdater(proj, dataset[selected_bag_id]->image2, dataset[selected_bag_id]->points));
    viewer->update_drawable("points", color_updater->cloud_buffer, guik::VertexColor());

    if (draw_sphere) {
      sphere_updater.reset(new PointsColorUpdater(proj, dataset[selected_bag_id]->image2));
      viewer->update_drawable("sphere", sphere_updater->cloud_buffer, guik::VertexColor());
    }

    if (show_image_cv) {
      cv::Mat canvas;
      cv::resize(dataset[selected_bag_id]->image2, canvas, cv::Size(), image_display_scale, image_display_scale);
      cv::imshow("image", canvas);
    } else {
      cv::Mat bgr=dataset[selected_bag_id]->image2;
      // cv::cvtColor(dataset[selected_bag_id]->image2, bgr, cv::COLOR_RGB2BGR);
